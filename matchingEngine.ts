...
import type { Character8D, Big5Scores, TagSelection } from './characterGenerator8D.js';
...
export class MatchingEngine {
  ...
  static calculateFetishOverlap(
    user1Tags: TagSelection[],
    user2Tags: TagSelection[]
  ): number {
    if (user1Tags.length === 0 || user2Tags.length === 0) return 0;

    const user1TagIds = new Set(user1Tags.map((t: TagSelection) => t.tagId));
    const user2TagIds = new Set(user2Tags.map((t: TagSelection) => t.tagId));
    ...
    const user1MustHaves = user1Tags.filter((t: TagSelection) => t.tagType === 'must');
    const user2MustHaves = user2Tags.filter((t: TagSelection) => t.tagType === 'must');
  ...
  static calculateAestheticHarmony(
    ...
    if (
      ...
      const modOverlap = l1.bodyModifications.filter((m: string) =>
